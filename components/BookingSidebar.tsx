...
export default function BookingSidebar({ tour }: { tour: Tour }) {
  ...
  // Availability lookup — re-runs whenever the user picks a new date.
  // `productId` is wired from NEXT_PUBLIC_BOKUN_PRODUCT_* at build time; if
  // it's missing we stay in `idle` and the submit button falls back to an
  // email CTA (see render below).
  useEffect(() => {
    ...
    fetch(
      ...
      .then(data => {
        ...
        const soldOut = data.availabilities.length > 0 && data.availabilities.every(a => a.soldOut)
